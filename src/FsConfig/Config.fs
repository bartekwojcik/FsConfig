...
module internal Core =
  ...
  let rec parseInternal<'T> (configReader : IConfigReader) (fieldNameCanonicalizer : FieldNameCanonicalizer) args =
    ...
    | Shape.FSharpOption fsharpOption -> 
      parseFSharpOption<'T> args.Name value fsharpOption configReader fieldNameCanonicalizer args
  ...
  and parseFSharpOption<'T> name value (fsharpOption : IShapeFSharpOption) configReader fieldNameCanonicalizer args =
    let wrap (p : ConfigParseResult<'a>) =
      unbox<ConfigParseResult<'T>> p
      ...
      new ITypeVisitor<ConfigParseResult<'T>> with
        member __.Visit<'t>() =
          ...
          | None ->
            match value with
            | None ->
                let targetShape = shapeof<'t>
                match targetShape with
                | Shape.FSharpRecord (:? ShapeFSharpRecord<'t> as shape) ->
              
                    let record  = parseFSharpRecord configReader fieldNameCanonicalizer (Prefix args.Name) shape
                    let result = record |> Result.bind (Some >> Ok >> wrap)
                    result
                | _ ->
